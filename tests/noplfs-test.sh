...
#
env LD_PRELOAD=$BUILD_PREFIX/src/libdeltafs-preload.so PDLFS_Deltafs_NoPLFS_test=1 \
    mpirun -np $MPI_PROCS -mca btl ^openib $BUILD_PREFIX/tests/noplfs-test

if [ $? != 0 ]; then
    echo "Deltafs No PLFS test failed ($?)"
    exit 1
fi

echo "Deltafs No PLFS test successful"
exit 0
